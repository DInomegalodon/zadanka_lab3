...
class BiesiadnaPizzaBuilder : public PizzaBuilder {
public:
	void buildDough() override
	{
		pizza.setDough("Grube Dough");
	}
	void buildSauce() override
	{
		pizza.setSauce("Double Tomato Sauce");
	}
	void buildTopping() override
	{
		pizza.setTopping("Kielbasa Boczek i Ogorek Kiszony");
	}
	Pizza getPizza() const override { return pizza; }
private:
	Pizza pizza;
...
int main()
{
	...
	// tworzenie biesiadnej pizzy 
	BiesiadnaPizzaBuilder biesiadnaBuilder;
	cook.makePizza(biesiadnaBuilder);
	Pizza biesiadna = biesiadnaBuilder.getPizza();
	biesiadna.displayPizza();
	return 0;
}